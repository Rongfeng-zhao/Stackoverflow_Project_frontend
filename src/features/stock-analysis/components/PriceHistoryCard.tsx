...
import type { StockPriceHistoryPoint } from '../model/stockAnalysis.domainTypes';
...
function buildPath(points: StockPriceHistoryPoint[], width: number, height: number, padding: number) {
  if (points.length === 0) {
    return '';
  }
...
export function PriceHistoryCard({ points, currency = 'USD' }: PriceHistoryCardProps) {
  ...
  return (
    <CardShell title="Price History" subtitle="Daily price history returned by the backend" className="overflow-hidden">
        ...
        <div className="space-y-5">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div>
              ...
              <p className="mt-1 font-display text-3xl font-semibold text-ink-900">
                {formatCurrency(latest.close, currency ?? 'USD')}
              </p>
            </div>
            <div className="grid grid-cols-2 gap-3 text-right sm:grid-cols-4">
              <div className="rounded-2xl bg-ink-50 px-3 py-2">
                <p className="text-xs text-ink-500">Open</p>
                <p className="text-sm font-semibold text-ink-900">{formatCurrency(latest.open, currency ?? 'USD')}</p>
              </div>
              <div className="rounded-2xl bg-ink-50 px-3 py-2">
                <p className="text-xs text-ink-500">High</p>
                <p className="text-sm font-semibold text-ink-900">{formatCurrency(latest.high, currency ?? 'USD')}</p>
              </div>
              <div className="rounded-2xl bg-ink-50 px-3 py-2">
              ...
              <div className="rounded-2xl bg-ink-50 px-3 py-2">
                <p className="text-xs text-ink-500">30-day change</p>
                <p
          ...
          <div className="rounded-3xl bg-gradient-to-br from-ink-900 to-ink-700 p-4">
            <svg viewBox="0 0 640 220" role="img" aria-label="Stock price history line chart" className="h-64 w-full">
              <defs>
                <linearGradient id="priceFillUs" x1="0" x2="0" y1="0" y2="1">
                  <stop offset="0%" stopColor="#38bdf8" stopOpacity="0.38" />
                  <stop offset="100%" stopColor="#38bdf8" stopOpacity="0" />
                </linearGradient>
              </defs>
              <path d={`${path} L 616 196 L 24 196 Z`} fill="url(#priceFillUs)" />
